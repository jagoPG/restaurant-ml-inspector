...
from src.application.command.review.edit_sentiment import \
    EditReviewSentimentCommand
from src.domain.exception import ReviewDoesNotExist, AccessViolation
from web.routing.actions.action import Action


class EditReviewSentiment(Action):
    def __init__(self):
    ...
    def invoke(self, request=None, **kwargs):
        ...
        command = EditReviewSentimentCommand(
            review_id, project_id, float(json_data['sentiment'])
        )
        try:
            self.command_bus.execute(command)
        except ReviewDoesNotExist:
            current_app.logger.warning(
                '{0} non existing review tried to change its sentiment'.format(
                    review_id
                )
            )
            raise NotFound
