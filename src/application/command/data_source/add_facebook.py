...
class AddFacebookSourceCommand(object):
    """
    Adds a new restaurant source from Facebook
    """
...
class AddFacebookSource(CommandHandler):
    def __init__(self):
    ...
    def invoke(self, command):
        ...
        facebook = self.__retrieve_social_network()
        self.__check_restaurant_not_exists(project, facebook)
        country = self.__retrieve_country(command.country)
        self.__create_restaurant_instance(command, project, country, facebook)
    ...
    def __retrieve_social_network(self):
        return self.social_network_repository.get_of_name('Facebook')
